    ...
    public class TeddyBear
    {
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (elapsedShotTime > firingDelay)
            {
                elapsedShotTime = 0;
                firingDelay = GetRandomFiringDelay();
                Projectile tprojectile = new Projectile(ProjectileType.TeddyBear, Game1.GetProjectileSprite(ProjectileType.TeddyBear),
                    drawRectangle.Center.X, drawRectangle.Center.Y - GameConstants.TEDDY_BEAR_PROJECTILE_OFFSET, GetProjectileYVelocity());
                Game1.AddProjectile(tprojectile);
                shootSound.Play();
            }
            ...
            int x, int y)
        {
            ...
            sprite = contentManager.Load<Texture2D>("teddybear");
